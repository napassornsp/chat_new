...
const ENDPOINT = "/vision/pet/detect";
...
export default function PetDetection() {
  const canonical =
    typeof window !== "undefined" ? window.location.origin + "/vision/pet-detection" : "";
  const title = useMemo(() => "Pet Detection", []);
  ...
  const drawOutput = () => {
    ...
    if (resp?.boxes?.length) {
      ...
      for (const b of resp.boxes) {
        ...
        const c = b.color || "#10b981"; // green default
        ctx.strokeStyle = c;
